...
@onready var ranged_comp: Node = npc.get_node("Components/BowComponent")
@onready var detection_area: Area2D = npc.get_node("DetectionArea")

@export var melee_range: float = 70.0
var current_target: Node2D = null

var target_out_of_range_timer: float = 0.0
...
# Evaluates targets and manages the out-of-range abandonment timer while executing combat logic.
func _process_combat_state(target: Node2D, delta: float) -> void:
	...
	var max_reach: float = attack_range if is_instance_valid(active_ranged) else melee_range
	if dist > max_reach:
		target_out_of_range_timer += delta
		if target_out_of_range_timer >= 10.0: # Gives up on trying to chase something 
